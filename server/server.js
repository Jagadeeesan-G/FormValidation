...
// app.post("/emp", (req, res) => {
//     const { name, employeeId, email, phoneNumber, department, dateOfJoining } = req.body;

//     if (!name || !employeeId || !email || !phoneNumber || !department || !dateOfJoining) {
//         return res.status(400).json({ message: "All fields are required!" });
//     }

//     const query = "INSERT INTO employee (name, employeeId, email, phoneNumber, department, dateOfJoining) VALUES(?,?,?,?,?,?)";

//     db.query(query, [name, employeeId, email, phoneNumber, department, dateOfJoining], (err, results) => {
//         if (err) {
//             console.error("Error occurred while processing:", err);
//             return res.status(500).json({ message: "Error occurred while processing the data", error: err });
//         }
